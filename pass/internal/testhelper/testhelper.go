...
func SetupTestEnv() (*TestEnv, error) {
	...
	env := &TestEnv{
		...
		OrigPassGPGID:     os.Getenv("PASS_GPG_ID"),
	}
...
// SetupTestEnvWithGPGKeys sets up a test environment with temporary GPG keys.
// This is useful for tests that need actual GPG encryption/decryption.
//
// The keys are generated on-the-fly inside the test GNUPGHOME
...
func (e *TestEnv) Cleanup() {
	...
	if e.OrigPassGPGID != "" {
		os.Setenv("PASS_GPG_ID", e.OrigPassGPGID)
	} else {
		os.Unsetenv("PASS_GPG_ID")
	}
...
func (e *TestEnv) ReadTestPassword(path, passphrase string) (string, error) {
	...
	args := []string{"--batch", "--pinentry-mode", "loopback", "--decrypt"}
